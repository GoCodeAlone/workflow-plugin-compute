...
func newProductCaptureStep(name string, raw map[string]any) (*productCaptureStep, error) {
	...
	if cfg.PollInterval != "" {
		if d, err := time.ParseDuration(cfg.PollInterval); err != nil {
			...
			return nil, fmt.Errorf("step.compute_product_capture %q: poll_interval must be positive", name)
		}
	}
	if cfg.WaitTimeout != "" {
		if d, err := time.ParseDuration(cfg.WaitTimeout); err != nil {
			return nil, fmt.Errorf("step.compute_product_capture %q: wait_timeout must be duration: %w", name, err)
		} else if d <= 0 {
			return nil, fmt.Errorf("step.compute_product_capture %q: wait_timeout must be positive", name)
		}
	}
...
func (s *productCaptureStep) waitForProductCapture(ctx context.Context, client *computeClient, taskID string) (map[string]any, error) {
	...
	for {
		...
		actionableStalls := waitConfig{RequireProof: s.config.RequireProof}.actionableStalls(stalls)
		if task.Status == protocol.TaskFailed || task.Status == protocol.TaskStalled || len(actionableStalls) > 0 {
			output := taskOutput(task)
			if len(actionableStalls) > 0 {
				addStallOutput(output, actionableStalls[0])
		...
		if isTerminalTaskStatus(task.Status) {
			...
			if hasProof || !(waitConfig{RequireProof: s.config.RequireProof}).requireProof() {
				return output, nil
			}
		}
		timer := time.NewTimer(pollInterval)
